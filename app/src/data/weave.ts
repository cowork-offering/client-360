...
/* =============================================================================
   THE GROUND BAND (founder, 2026-09-03): the coarse strand the refraction
   needs, and the ONLY reason it exists.

   Refraction bends what is behind a panel. Blur runs first in the eye's book:
   an 18px average erases a 1px filament completely, so a bend over the weave
   as it stands moves nothing. The band is structure at the BEND's own
   wavelength (about 167px at baseFrequency 0.006) rather than at the
   filaments' 1px: four strands 6 to 10px wide at 3 to 5 percent alpha, which
   survive the average and give every glass surface an edge to displace.

   IT IS A SEPARATE FUNCTION, NOT A BRANCH INSIDE buildThreads, and that is not
   a style preference. buildThreads is a transcription of the dummy's generator
   whose draw ORDER is the picture; one extra rnd() inside it moves all twelve
   filaments. The band therefore runs its own LCG off its own seed and cannot
   perturb the mint.

   STILL, on purpose. The filaments breathe because they are a feature of the
   landing; the band is ground, and a ground that moves would put a rAF loop
   under every view for a texture nobody is meant to notice.
   ============================================================================= */

const BAND_SEED = 20260903;
const BAND_N = 4;

/** The band's own space. Square and stretched by `preserveAspectRatio="none"`,
 *  so one generated shape covers every viewport the cockpit runs at. */
export const BAND_VIEWBOX = "0 0 1000 1000";

export function buildBand(): Thread[] {
  let s = BAND_SEED;
  const rnd = () => {
    s = (s * 1103515245 + 12345) & 0x7fffffff;
    return s / 0x7fffffff;
  ...
  for (let i = 0; i < BAND_N; i++) {
    /* Spread down the viewport with a little jitter, so the strands cross the
       hero, the arc and the rail at different heights rather than banding. */
    const y = 130 + (i / (BAND_N - 1)) * 700 + (rnd() * 2 - 1) * 60;
    const a1 = (rnd() * 2 - 1) * 150;
    const a2 = (rnd() * 2 - 1) * 170;
    const a3 = (rnd() * 2 - 1) * 140;
    out.push({
      d:
        "M -60 " + (y + a1 * 0.4).toFixed(1) +
        " C 240 " + (y + a1).toFixed(1) + " 430 " + (y + a2).toFixed(1) + " 620 " + (y + a2 * 0.5).toFixed(1) +
        " S 900 " + (y + a3).toFixed(1) + " 1060 " + (y + a3 * 0.3).toFixed(1),
      stroke: CORE,
      width: (6 + rnd() * 4).toFixed(2),
      opacity: (0.03 + rnd() * 0.02).toFixed(3),
      amp: 0,
      ax: 0,
      freq: 0,
      phase: 0,
