...
| `cockpit.json` | `canonicalArtifactUrl`: the cockpit an intent is written to by a session that did not publish one itself | hand-edited, once per canonical republish |
...
next-step action ids drift from the sets `render/contract-checks.mjs` actually enforces. Both are
regenerated by running the same script without `--check`.
